...
<ww:set name="billdetail" value="%{getSelectedBillDetailIds('${bill.id}','17')}"/> 
...
<display:table name="billdetail" class="list"  id="row" export="false" pagesize="${GLOBAL_pagesize}"  sort="list"  requestURI="uploadStep1.html"  >
    ...
    <display:column titleKey="common.action" >
					 ...
					 <fmt:message key='billSchedule.step11'/>:${fn:substring(schedule.check11,0,16)}/
           <fmt:message key='member.name'/>:${schedule.checker11}/
           <fmt:message key='billSchedule.sendMethod'/>:${schedule.isSend}/
           <fmt:message key='freight.name'/>:${schedule.freight.name}/
           <fmt:message key='freight.no'/>:${schedule.freightNo}/
           <fmt:message key='billSchedule.arrival'/>:${schedule.arrival}
...
if (document.Form1.sendMethod.value == "5"){		
...
function isSendId(){
	if (document.Form1.sendMethod.value != "5"){		
		document.Form1.freightNo.value="";
		document.Form1.freight.value="";
		document.Form1.freightNo.disabled=true;
		document.Form1.freight.disabled=true;		
	}else{
		document.Form1.freightNo.disabled=false;
		document.Form1.freight.disabled=false;	
	}
} 

 <ww:set name="test" value="%{getBillDetailListByBillNo('${bill.id}','17')}"/>
 <ww:iterator value="test"> 
   <ww:set name="s" value="%{getBillScheduleById(id)}"/>
    <ww:if test="${s.step11} == true">     
