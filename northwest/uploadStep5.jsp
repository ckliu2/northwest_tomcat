...
<ww:form name="Form1" action="saveCheck5" method="POST">
  <ww:hidden name="bill.id" value="${bill.id}"/>
   <table border="0" cellspacing="1" class="cContentTable1">
    ...
    <tr class="cActionButtonLine">
     ...
     <tr><td class="cInputCaption"><fmt:message key="billSchedule.step2-1"/></td>
        <td class="cInputColumn">
            <ww:textfield  name="check2" id="check2"  readonly="true" value="%{billSchedule.check2}"/>
            <img src="images/cal.gif" onclick="javascript:NewCssCal ('check2','yyyyMMdd','dropdown',true,'24')"  style="cursor:pointer"/>
        </td>
    </tr>
    ...
    <tr><td class="cInputCaption"><fmt:message key="billSchedule.check5"/></td>
        <td class="cInputColumn">
            <ww:textfield  name="check5" id="check5"  readonly="true" value="%{billSchedule.check5}"/>
            <img src="images/cal.gif" onclick="javascript:NewCssCal ('check5','yyyyMMdd','dropdown',true,'24')"  style="cursor:pointer"/>
        </td>
    </tr>
        ...
        <td class="cInputColumn">
            ...
            <tr>
                ...
                <ww:if test="bill.id != null">
                   ...
                   list="%{getBillDetailListByBillNo('${bill.id}','10')}"
                   listKey="id"
                ...
                <ww:else>
                   ...
                   <ww:select id="allBillDetail"
                   list="%{getBillDetailListByProducts('10')}"
                   listKey="id"
                   listValue="detail+'-'+product.productName+'-'+width+'X'+heigh+'('+visibleWidth+'X'+visibleHeigh+')-'+quantity+'-'+itemRemark"               
                   multiple="true"
                   size="8"
...
<ww:set name="billdetail" value="%{getSelectedBillDetailIds('${bill.id}','10')}"/> 
...
<display:table name="billdetail" class="list"  id="row" export="false" pagesize="${GLOBAL_pagesize}"  sort="list"  requestURI="uploadStep1.html"  >
    ...
    <display:column titleKey="common.action" >
             <fmt:message key='billSchedule.step2'/>:${fn:substring(schedule.check2,0,16)}/
             <fmt:message key='member.name'/>:${schedule.checker2}
             <br>
             <fmt:message key='billSchedule.step5'/>:${fn:substring(schedule.check5,0,16)}/
             <fmt:message key='member.name'/>:${schedule.checker5}            
    </display:column> 
...
{	
	/*
  if (document.Form1.check5.value == ""){
	  alert('<fmt:message key="billSchedule.check5"/><fmt:message key="common.required"/>');	       
	  return False;
	}		
	*/
 ...
 <ww:set name="test" value="%{getBillDetailListByBillNo('${bill.id}','10')}"/>
 <ww:iterator value="test"> 
   <ww:set name="s" value="%{getBillScheduleById(id)}"/>
    <ww:if test="${s.step5} == true">     
